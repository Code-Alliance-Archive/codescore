...
*/
	...
	$testq1 = mysql_query("SELECT survid FROM " . $tabloc . " WHERE survid != ''");
	...
	if($testq1 == ''){
		...
		$testq2 = mysql_query("SELECT MAX(survid) FROM " . $tabloc);
	...
	foreach($data as $k => $v){
		...
		if($k != "tablelocation"){
			...
			if(strpos($k, 'wt') == false){
				$finalhalf .= "'" . mysql_real_escape_string($k) . ":" . mysql_real_escape_string($v) . "'" . ",";
			}
			//if we are adding a weight, value is an integer
			else{
